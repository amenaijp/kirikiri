...
async fn main() -> Result<()> {
    ...
    println!("Listening for internal http requests at http://kirikiri.local");

    let app = Router::new().route("/", get(|| async { "kirikiri internal server" }));

    loop {
        match listener.accept().await {
            Ok((socket, _client_addr)) => {
                spawn_and_log_error(serve_socks5(opt, socket, app.clone()));
            }
            Err(err) => {
...
async fn serve_socks5(opt: &Opt, socket: TcpStream, app: Router) -> Result<(), SocksError> {
    ...
    println!("Received request to connect to {:?} ", &target_addr);

    if let TargetAddr::Domain(ref domain, _) = target_addr {
        if domain == "kirikiri.local" {
            println!("!!!handling admin panel");
            let inner = proto
                .reply_success(SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 0))
                .await?;

            tokio::spawn(async move {
                let socket = TokioIo::new(inner);
                let hyper_service =
    ...
    return if let TargetAddr::Ip(resolved_addr) = target_addr.resolve_dns().await? {
        println!("Resolved address to {:?}, proxying...", resolved_addr);
        let mut outbound =
            tokio::time::timeout(opt.request_timeout, TcpStream::connect(resolved_addr))
                .await
        ...
        match buf[0] {
            ...
            0x16 => {
                ...
                println!("successfully segmented: {}", successfully_segmented);
        ...
        tokio::io::copy_bidirectional(&mut inbound, &mut outbound).await?; // will error out on broken pipe
        Ok(())
    } else {
